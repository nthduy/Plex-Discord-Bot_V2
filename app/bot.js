...
const PLEX_PLAY_START =
  (plexConfig.https ? "https://" : "http://") +
  plexConfig.hostname +
  ":" +
  plexConfig.port;
...
class Bot extends EventEmitter {
  ...
  async findRandomTracksOnPlex(query, message) {
    if (Object.keys(this.cache_library).length === 0) {
      await this.loadLibrary();
    }
    const nombre = getRandomNumber(Object.keys(this.cache_library).length);
    const res = await this.plex.query(
      "/library/sections/" +
        this.cache_library[Object.keys(this.cache_library)[nombre]].key +
        "/all?type=10"
    );

    let songs = Array.from(Array(Number(query)).keys());
  ...
  async loadMood(id) {
    const res = await this.plex.query(
      "/library/sections/" + id + "/mood?type=10"
    );
    const moods = {};
    ...
    return Object.keys(moods).length === 0 && moods.constructor === Object
      ? undefined
      : moods;
  }
      ...
      if (id === -1) {
        ...
        for (let library of res.MediaContainer.Directory) {
          if (library.type === "artist") {
            this.cache_library[library.key] = {
              name: library.title,
              key: library.key,
              mood: await this.loadMood(library.key),
            };
        ...
        if (!res.MediaContainer.title1) {
          this.emit("loadLibraryError", {
            key: id,
            err: new Error("Key doesn't exist."),
          });
          return;
        }
        this.cache_library[id] = {
          name: res.MediaContainer.title1,
          key: id,
          mood: await this.loadMood(id),
        };
  ...
  async findArtist(name, message) {
    ...
    const resAlbums = await this.plex.query(
      resArtist.MediaContainer.Metadata[0].key
    );
  ...
  async playOneMood(moodName, message) {
    ...
    if (musics.length > 0) {
      const musicChosen =
        musics[Math.floor(Math.random() * Math.floor(musics.length))];
      ...
      if (!this.isPlaying) {
        ...
        message.reply(
          language.BOT_ADDTOQUEUE_SUCCES.format({
            artist: musicChosen.artist,
            title: musicChosen.title,
          })
        );
      }
    } else {
    ...
    try {
      ...
      if (resultSize === 1 && offset === 0) {
        ...
        for (let t = 0; t < this.tracks.length; t++) {
          ...
          messageLines +=
            language.BOT_FIND_SONG_INFO_MUSIC.format({
              index: t + 1,
              artist: artist,
              title: this.tracks[t].title,
            }) + "\n";
        }
        messageLines += language.BOT_FIND_SONG_INFO.format({
          caracteres_commande: this.config.caracteres_commande,
        });
    ...
    if (this.voiceChannel) {
      ...
      if (this.workingTask > 0) {
        ...
        this.dispatcher = connection
          .play(Readable.from(readstream.body, { highWaterMark: 20971520 }))
          .on("finish", dispatcherFunc)
          .on("start", () => {
  ...
  songToEmbedObject(song) {
    return {
      embed: {
        ...
        footer: {
          text: language.NUMBER_MUSIC_IN_QUEUE.format({
            number: this.songQueue.length,
            plurial: this.songQueue.length > 1 ? "s" : "",
          }),
        },
      },
    };
  ...
  async ajoutPlaylist(nomPlaylist, musique, message) {
    const nomFichier =
      this.config.dossier_playlists + nomPlaylist + ".playlist";
    ...
    fs.readFile(nomFichier, "utf8", async (err, data) => {
      ...
      fs.writeFile(nomFichier, json, "utf8", async (err, written, string) => {
        ...
        message.reply(
          language.ADD_SONG_TO_PLAYLIST_SUCCES.format({
            title: musique.titre,
            artist: musique.artiste,
            playlist_name: playlist.nom,
          })
        );
      });
    });
